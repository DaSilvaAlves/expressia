...
|----------|-------|----------------|-----|
| `DATABASE_URL` | 6543 | Transaction-mode | Runtime queries com utilizador final (`getDb()`, role `authenticated`, RLS aplicada) |
| `DATABASE_URL_DIRECT` | 5432 | Session-mode | Migrations, `drizzle-kit`, scripts admin (alias histórico nos runbooks: `DIRECT_URL`) |
| `DATABASE_URL_SERVICE_ROLE` | 5432 | Session-mode | Jobs sistémicos RLS-bypass (`getServiceDb()`, role `postgres`): crons Inngest, GDPR purge, scripts admin. Tipicamente igual ao `DATABASE_URL_DIRECT`. |
...
Em produção (Vercel), o runtime Next.js usa `DATABASE_URL` (pooler 6543
transaction-mode) para queries com utilizador final. Jobs sistémicos que
correm como funções Inngest no mesmo runtime (ex. cron
`generate-recurring-tasks`, `cleanup-expired-reverse-ops`) precisam de
`DATABASE_URL_SERVICE_ROLE` — connection RLS-bypass consumida por
`getServiceDb()`. `DATABASE_URL_DIRECT` continua a ser usada em CI/CD para
correr migrations via GitHub Actions com approval gate (Architecture §11.4).

| Env | Onde | Quando |
|-----|------|--------|
| `DATABASE_URL` | Vercel runtime | Cada request runtime (`getDb()`, role `authenticated`) |
| `DATABASE_URL_SERVICE_ROLE` | Vercel runtime (Production + Preview) | Jobs Inngest e scripts (`getServiceDb()`, role `postgres`, RLS-bypass). Tipicamente igual ao `DATABASE_URL_DIRECT`. |
| `DATABASE_URL_DIRECT` | GitHub Actions secrets | Apenas no job `db-migrate` |
| `SUPABASE_SERVICE_ROLE_KEY` | Vercel + GitHub | API REST Supabase admin (distinto da connection Postgres directa) |
