...
**Criado:** 2026-05-08
**Actualizado:** 2026-05-09 (Story 2.5 **Done v1.4** — `@devops *push 2.5` completo. 5 commits pushed `origin/main` em range `970f19b..51467fe` (b7ab5a2 + acb949f + 3e3d91e + 642e0e9 + 51467fe). 5 quality gates re-verificados pelo @devops em FULL TURBO cache hit (typecheck 9/9 80ms, lint 9/9 75ms 0 warnings, test 9/9 75ms — 416 tests preservados, check:rls 747ms — 26 tabelas / 104 policies NFR5 PRESERVADA, build 9/9 83ms — Next.js 9 routes + Sentry sourcemap 3 runtimes). [AUTO-DECISION] CodeRabbit pre-push skipped consistente com 2.2/2.3/2.4 D14 — architect gate APPROVED 9.4/10 HIGH ~700 linhas substitui review automatizado. Housekeeping completo: story file `git mv active/ → completed/` (history preservado), `_INDEX.md` row 2.5 InReview → Done + cabeçalho Epic 2 4/11 → 5/11, story v1.3 → v1.4 com entry @devops push, handoff `mj-handoff-2.5-ready-for-devops-close-20260509` consumed + `git mv → archive/`, `HANDOFF-INDEX.md` pending row removed + archived row added. **Story 2.5 Done. Epic 2 — 5/11 stories Done (2.1 + 2.2 + 2.3 + 2.4 + 2.5).** Story 2.6 (Endpoint POST /api/agent/prompt) unblocked estruturalmente — depende 2.2 ✓ + 2.3 ✓ + 2.4 ✓ + 2.5 ✓; EB1 PENDING não bloqueia mockable-only. Sem sucessor handoff automático — drafting de 2.6 fica a critério de Eurico.
