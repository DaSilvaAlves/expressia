...
**Criado:** 2026-05-08
**Actualizado:** 2026-05-08 (Story 2.4 **Ready** ✅ — `@po *validate-story-draft 2.4` GO **9.4/10 high confidence**. 10/10 critérios PASS + executor assignment válido (@dev ≠ @architect) + anti-hallucination cross-confirmado contra 3 fontes (enum DB `agent_intent` 8 valores match exacto, architecture.md §4.2 `IntentSchema`/`ClassificationSchema` literal, FR1+FR4+CON3 PRD). 1 minor fix inline aplicado pelo @po: AC3 trace correction — `intents.max(5)` reclamava trace para FR3 mas FR3 do PRD é audit log, não limit; corrigido para `[AUTO-DECISION D8]` explícita anti-hallucination guardrail (architecture.md:327 só especifica `min(1)`; PRD AC1 Epic 2 ilustra cenário com 2 intents canónicas; max(5) defensável como prudência mas validável em @architect gate). Status Draft → Ready (autoridade @po em GO verdict). Story v1.1. Implementação **NÃO bloqueada por EB2** — toda a story foi desenhada 100% mockable (padrão 2.2/2.3). Anterior: Story 2.3 **Done** ✅ formal — `@devops *push` completo: 7 commits pushed para `origin/main` (range `1264606..26af717`), 5 quality gates exit 0 re-verificados pelo @devops antes do push, story file movida `active/` → `completed/`. **Epic 2 — 3/11 stories Done + 1 Ready (2.4).** Próximo passo: `@dev *develop 2.4` em modo mockado. 2.5 (Planner+Executor) ainda bloqueada por EB1 + Story 2.4 Done.)
**Autor:** River (@sm)
...
| 2.4 | [2.4.classifier-pt-pt.md](./2.4.classifier-pt-pt.md) | Classifier PT-PT (GPT-4o-mini) + Zod gate | **Ready** ✅ (v1.1 — @po GO 9.4/10 high; @dev *develop 2.4 unblocked) | @dev | M | ~~Depende 2.2~~ ✅ — validada 2026-05-08 por @po; minor fix AC3 inline; EB2 pending só afecta integração real (Story 2.10), implementação mockada **NÃO bloqueada** |
