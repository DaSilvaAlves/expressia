...
**Criado:** 2026-05-08
**Actualizado:** 2026-05-09 (Story 2.6 **Ready v1.1** — `@po *validate-story-draft 2.6` veredicto **GO 9.3/10 HIGH** confidence. 10/10 critérios PASS + executor assignment válido (@dev — endpoint implementation, NÃO @architect) + anti-hallucination cross-confirmado contra 11 fontes (Architecture §1.3/§4.1/§4.4/§4.5/§7.2/§7.3/§9 + db-schema.md §4.4 + packages/db/src/schema/agent.ts + 5 stories completed 2.1/2.2/2.3/2.4/2.5 + PRD FR1-FR6/NFR5/NFR9/NFR12/NFR13/NFR16/NFR17/NFR19) + 8 [AUTO-DECISIONS] D17-D24 reviewed (7 PASS + 1 CONCERNS-ACCEPTABLE: D21 path normalization deviation — @architect ratifica). **5 [PO_FIX_INLINE] aplicados** correcting hallucinations vs schema real: (1) AC7+T10 `reverted_at IS NULL` → `executed_at IS NULL` em `agent_reverse_ops` (real column per schema agent.ts:202 + db-schema.md §4.4 invariantes; architecture §4.5 tem stale `reverted_at` — DOC-001-NB do gate 2.3 nunca corrigido); (2) AC11+T6 `redactPii()` from @meu-jarvis/agent → 3 opções deferidas a @dev/@architect (sanitizeHint exportado / impl local / D9-redactPii re-export — `redactPii` NÃO está exportado, só `sanitizeHint`; `redactProviderPayload` é privado intencional per index.ts:12); (3) AC10+T2+T5+T9 `intent_classification` (singular invented) → `intents_detected` (real column plural agent.ts:84); (4) AC12+T11 `llm_model` (single invented) → `classifier_model` + `executor_model` (DUAS colunas reais agent.ts:93-94); (5) T8 `quota_limit` → plano tier-derived limit (coluna `quota_limit` NÃO existe em `agent_quotas`). Status Draft → Ready (autoridade @po em GO verdict per `story-lifecycle.md`). Story v1.0 → v1.1 com PO Validation block + Change Log @po. **EB1/EB2/EB3 PENDING NÃO bloqueiam** — implementação @dev pode arrancar mockable-only padrão estabelecido em 2.2/2.3/2.4/2.5 (4 stories Done APPROVED 9.4-9.5/10). Próximo passo: `@dev *develop 2.6` modo YOLO. **Epic 2 — 5/11 stories Done (continua); Story 2.6 Ready em progresso.**
...
| 2.6 | [2.6.endpoint-prompt-canonical-consumer.md](./2.6.endpoint-prompt-canonical-consumer.md) | Endpoint POST /api/agent/prompt — Consumidor Canónico do Pipeline AI | **Ready for Review** 🔍 (v1.2 — @dev YOLO complete; awaits architect gate) | @dev → @architect | M | Draft @sm 2026-05-09 v1.0 → Ready @po v1.1 → InProgress @dev → Ready for Review v1.2 (15/15 Tasks done, 5/5 quality gates GREEN: typecheck 9/9, lint 9/9 zero warnings, test 480 = 64 novos + 416 preservados, check:rls 27/108, build Next.js 4 dynamic routes incluindo 3 novas /api/agent/prompt). 3 [AUTO-DECISIONS] @dev (D25 redactPii local opção b, D26 CodeRabbit skipped, D27 cross-package import refactor) + 1 [DEV-DECISION] D21 path nested mantido. Migration 0006 aplicada. Próximo: `@architect *qa-gate 2.6`. |
