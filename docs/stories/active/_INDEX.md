...
**Criado:** 2026-05-08
**Actualizado:** 2026-05-08 (Story 2.3 v1.2 — `@dev *develop` **completo**. Package `@meu-jarvis/tools` criado em 16 ficheiros (7 src + 5 tests + README + 3 config) com 57 testes pass (target ≥30, quase 2× cobertura). 4 commits sequenciais (`d38c0c8` scaffold + `eb1ae30` core + `b745de2` tests + `d79fcef` README). 5 quality gates exit 0: typecheck (7/7 packages), lint (--max-warnings=0 OK), test (57 novos + 222+ existentes preservados), build (Next.js 54s), check:rls (26 tabelas / 104 policies — NFR5 inalterada). 6 decisões arquitecturais D1-D6 documentadas para review @architect. 3 PO suggestions endereçadas inline (locus `ToolPlanGateError`, composite per-level recursivo, idempotência por referência). EB1/EB2 NÃO necessários (story 100% mockable). Status Ready → Ready for Review. Próximo passo: `@architect *qa-gate 2.3`. Anterior: Story 2.3 v1.1 — `@po` GO 9.7/10 HIGH; Story 2.2 v1.3 **Done** ✅.)
**Autor:** River (@sm)
...
| 2.3 | [2.3.tool-registry-foundation.md](./2.3.tool-registry-foundation.md) | Tool Registry Foundation (contrato + registry + executeAtomic) | **Ready for Review** ✅ (v1.2 — @dev complete, awaiting @architect gate) | @dev | L | ~~Depende 2.2~~ ✅ — entregue 2026-05-08 (4 commits, 57 tests pass, 5 gates exit 0; handoff `mj-handoff-2.3-ready-for-architect-gate-20260508`) |
