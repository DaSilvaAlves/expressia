...
| 2.7 | [2.7.preview-then-confirm-flow-ui.md](./2.7.preview-then-confirm-flow-ui.md) | Preview-then-confirm flow (FR4) + UI Chat Mínima | **Done** ✅ v1.3 (awaits push) | @devops | L (scope-shift M→L per D28) | ~~Depende 2.6~~ ✓ — drafted 2026-05-09 by Orion (@aiox-master delegated @sm); validated 2026-05-09 by Pax (@po) **GO 9.0/10 HIGH**; implemented 2026-05-09 by Dex (@dev) YOLO mockable-only — 14/14 Tasks done, 5/5 gates GREEN, 543 tests; **architect gate 2026-05-09 by Aria (@architect) APPROVED 9.4/10 HIGH** — 14/14 ACs PASS (3 EXCEEDS), 10/10 D28-D37 ratificadas (D28 RATIFIED scope-shift M→L cross-domain com policy futura, D29-D34 PASS, D35 ACCEPTED cosmético, D36 ACCEPTED NFR16 partial-deferral via Pino NIT-001-NB, D37 ACCEPTED prefer-migration-0008-defensiva NIT-002-NB), 5 gates re-verified (28/120 ✓ user_prefs), Constitution 6/6, 10 NFRs cobertos. **DOC-FIX-002 aplicada inline** em architecture.md §4.4 (two-override-paths preview + schema user_prefs + lazy-init D32 + run_id substitui HMAC) + db-schema.md §4.8 (nova secção user_prefs com migration split + predicate combo + lazy-init). 4 NITs deferred (NIT-001 audit enum, NIT-002 migration 0008, NIT-003 parser cosmetic, NIT-004 components extraction threshold). Próximo: `@devops *push 2.7` (sub-task NIT-002-NB criar 0008_user_prefs_rls.sql defensiva antes do push para prod EU). |
