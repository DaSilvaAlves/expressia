...
**Criado:** 2026-05-08
**Actualizado:** 2026-05-09 (Story 2.7 **Draft v1.0** criada — Orion (@aiox-master) delegated @sm authority per Constitution Article II universal executor (precedente Story 2.2) draftou Story 2.7 expandida com scope-shift M → L documentado em [AUTO-DECISION D28] (backend toggle + UI chat mínima `/jarvis`). Decisão estratégica de Eurico em sessão 2026-05-09 pós-Story 2.6 push: "ver o cérebro a funcionar" requer UI chat real (não só landing "Em construção"); EPIC-2 plan original cobria apenas backend toggle (M); Eurico escolheu Opção A "Story 2.7 expandida" em detrimento de Opção B (split @pm) / C (UI demo throwaway) / D (pivot mini-Epic UI Phase 1). Story 2.7 v1.0: 14 ACs (5 backend AC1-AC5 schema + endpoints + refactor /api/agent/prompt; 5 frontend AC6-AC10 page /jarvis + PreviewCard + /conta/preferencias; 4 testing AC11-AC14 Vitest backend + UI components ≥45 tests). 14 Tasks (T1-T4 backend, T5-T11 frontend, T12-T14 testing+gates). 7 [AUTO-DECISIONS] @sm D28-D34: D28 scope-shift M→L (@architect ratifica no gate), D29 schema 1:1 user (não user×household), D30 run_id-based (sem HMAC — leverage Story 2.6 D21+DOC-FIX-001), D31 components inline `_components/` (sem `components/` directory geral), D32 lazy-init `user_prefs` no GET (sem seed migration), D33 CodeRabbit skip consistente 2.2-2.6, D34 tests UI Vitest+jsdom (sem Playwright E2E). Estimate ≥45 testes target. Quality gates target 5/5 GREEN: typecheck 9/9, lint 9/9 0 warnings, test ≥525 (480 preservados + 45 novos), check:rls 28/112 NFR5 PRESERVADA + acrescida com `user_prefs` (1 nova tabela + 4 RLS policies idempotentes via DO block), build Next.js 4 dynamic + 2 novas routes (`/jarvis` + `/conta/preferencias`). Sem bloqueadores externos (EB1/EB2/EB3 não afectam — mockable-only padrão). 14 ficheiros novos + 6 modificados ≈ 20 ficheiros tocados. Próximo passo: `@po *validate-story-draft 2.7` (10-point checklist + cross-confirm anti-hallucination contra schema real `users_prefs` shape + ratificação D28-D34). Sucessor handoff: `mj-handoff-2.7-ready-for-po-20260509` (sm → po). **Epic 2 — 6/11 stories Done; Story 2.7 Draft em progresso.**
...
| 2.7 | [2.7.preview-then-confirm-flow-ui.md](./2.7.preview-then-confirm-flow-ui.md) | Preview-then-confirm flow (FR4) + UI Chat Mínima | **Draft** v1.0 | @dev → @architect | L (scope-shift M→L per D28) | ~~Depende 2.6~~ ✓ — drafted 2026-05-09 by Orion (@aiox-master delegated @sm); 14 ACs (5 backend + 5 frontend + 4 testing); 14 Tasks; 7 [AUTO-DECISIONS] D28-D34. Próximo: `@po *validate-story-draft 2.7` |
