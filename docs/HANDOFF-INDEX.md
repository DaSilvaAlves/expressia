...
|---------|-----|-----------|------------|-------|
| 2026-05-06 | [mj-handoff-vercel-setup-c5-blocker-20260506](handoffs/mj-handoff-vercel-setup-c5-blocker-20260506.yaml) | devops (Gage) → any (Eurico→Gage→Dex) | **Story 1.6 Tasks 4-5 bloqueadas por Vercel project não existir** | Tasks 1-3+6 done por Dex (commit local `64ae35d`, lint/typecheck/test verde 9/9). Tasks 4 (Playwright E2E) e 5 (Vercel perf) ⏸ paused. Vercel project NÃO existe para `DaSilvaAlves/expressia` (vercel projects ls: 22 projects no team mas nenhum ligado). Zero secrets no repo (CI passou porque Next.js build não falha sem env vars em build-time). **Playbook ~10 min Eurico + 2 min Gage:** P1 Eurico cria Vercel project via UI (Root: `apps/web`, Region: fra1, env vars Supabase) + P2 Eurico fornece SUPABASE_SERVICE_ROLE_KEY + P3 Gage `gh secret set` 3 secrets + P4 Dex retoma Tasks 4-5 + P5 QA gate + P6 Gage push. Decisão técnica C4: Opção B (Vercel GitHub Integration nativa, sem Vercel CLI no CI, action `bobheadxi/deployments@v1`). |
