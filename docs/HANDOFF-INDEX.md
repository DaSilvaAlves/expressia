...
|---------|-----|-----------|------------|-------|
| 2026-05-06 22:37 | [mj-handoff-vercel-middleware-crash-20260506](handoffs/mj-handoff-vercel-middleware-crash-20260506.yaml) | dev (Dex) → any (dev/devops, cabeça fresca) | **Vercel deploy verde mas middleware crash em runtime** | Sessão 21:30-22:37 com Eurico montou Vercel project Expressia correctamente (project name `expressia`, Root Directory `apps/web`, Region `cdg1` Paris UE, 5 env vars Supabase carregadas, 6 deploys Ready). Mas `expressia-black.vercel.app` retorna 500 `MIDDLEWARE_INVOCATION_FAILED` consistentemente. Sessão pausada — Eurico exausto, mais investigação requer terminal/codebase, não Vercel UI. **Hipóteses ordenadas:** H1 valores env vars com aspas/whitespace (HIGH), H2 turbo.json não declara `SUPABASE_SERVICE_ROLE_KEY` em `tasks.build.env` (MEDIUM), H3 `@supabase/ssr` Edge Runtime incompat (LOW), H4 versão @supabase/ssr quebrada (LOW). **Playbook próxima sessão:** P1 Dex lê runtime logs (não build) → decide hipótese, P2 Dex altera turbo.json, P3 Eurico valida valores se H1, P4 Gage push, P5 Dex retoma Tasks 4-5. |
| 2026-05-06 | [mj-handoff-vercel-setup-c5-blocker-20260506](handoffs/mj-handoff-vercel-setup-c5-blocker-20260506.yaml) | devops (Gage) → any (Eurico→Gage→Dex) | **Story 1.6 Tasks 4-5 bloqueadas por Vercel project não existir** | **PARCIALMENTE consumido em 2026-05-06 22h** — Passo 1 (Eurico cria Vercel project) FEITO mas com middleware crash residual. Ver `mj-handoff-vercel-middleware-crash` para próximos passos. Tasks 4 (Playwright E2E) e 5 (Vercel perf) continuam ⏸ paused até deploy verde end-to-end. Tasks 1-3+6 done por Dex (commit local `64ae35d`). |
