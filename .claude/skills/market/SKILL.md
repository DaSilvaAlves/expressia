...
**MANDATORY:** All outputs MUST be in Portuguese (PT-PT). Follow the project's `language-standards.md` rules:
- Use PT-PT vocabulary (never PT-BR): "utilizar", "ficheiro", "equipa", "eliminar"
- Date format: DD/MM/YYYY
- Number format: 1.500,00 (dot for thousands, comma for decimals)
- Technical terms stay in English: deploy, build, commit, pipeline, token, endpoint
- Tone: direct, practitioner-level — never academic or salesy
- Refer to `marketing-suite-rules.md` for scope boundaries with AIOX agents
